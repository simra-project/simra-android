...
class OBSService: Service() {
    ...
    val callback = object : BluetoothGattCallback() {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    ...
                    Handler(Looper.getMainLooper()).post {
                        Toast.makeText(applicationContext, getString(R.string.obsServiceConnectionSuccessful), Toast.LENGTH_LONG).show()
                    ...
                    Handler(Looper.getMainLooper()).post {
                        Toast.makeText(applicationContext, getString(R.string.obsServiceDisconnectionSuccessful), Toast.LENGTH_LONG).show()
                    }
                    gatt.close()
                    blestate = BLESTATE.START
                }
                ...
                Handler(Looper.getMainLooper()).post {
                    Toast.makeText(applicationContext, getString(R.string.obsServiceConnectionFailed).replace("ERRORCODE", status.toString()).replace("DEVICEADDRESS", deviceAddress), Toast.LENGTH_LONG).show()
                }
                result = null
                stopBleScan()
                gatt.close()
        ...
        override fun onServicesDiscovered(bluetoothGatt: BluetoothGatt, status: Int) {
            this@OBSService.gatt = bluetoothGatt
